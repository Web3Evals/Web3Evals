...
![Bar chart of expected golden bug groups from one run per repository, per condition](./fig1.svg "Figure 1. Expected golden bug groups from one run per repository.")
...
![Scatter of union versus majority recall for the ten prompt conditions](./fig2.svg "Figure 2. Breadth and repeatability have different winners.")
...
![Line chart of expected groups found as one, two, or three runs are merged, for C3 and C8](./fig3.svg "Figure 3. C8 overtakes C3 only when three runs are unioned.")
...
![Grouped bar chart of listed-class versus omitted-class recall per condition](./fig4.svg "Figure 4. Naming vulnerability classes did not raise recall on those classes.")
...
![Scatter of prompt token count against union recall for all ten conditions](./fig5.svg "Figure 5. Prompt length does not buy recall.")
...
![Scatter of cost per run against expected groups per run, with the cost-performance frontier marked](./fig6.svg "Figure 6. C3 sits on the cost–performance frontier.")
...
@misc{web3evals2026systemprompt,
  ...
  author = {web3evals},
  year   = {2026},
  url    = {https://www.web3evals.com/blog/what-a-system-prompt-actually-changes/},
}
```
